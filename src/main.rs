...
use spotify_iv2::playback::mixer::alsamixer::AlsaMixer;
...
fn setup_logging(quiet: bool, verbose: bool) {
    ...
    match env::var("RUST_LOG") {
        ...
        Err(_) => {
            if verbose {
                builder.parse_filters("libmdns=info,spotify_iv2=trace");
            } else if quiet {
                builder.parse_filters("libmdns=warn,spotify_iv2=warn");
            } else {
                builder.parse_filters("libmdns=info,spotify_iv2=info");
            }
            builder.init();
...
fn get_version_string() -> String {
    ...
    format!(
        "spotify_iv2 {semver} {sha} (Built on {build_date}, Build ID: {build_id}, Profile: {build_profile})",
...
fn get_setup() -> Setup {
    ...
    .optflag(
        VERSION_SHORT,
        VERSION,
        "Display spotify_iv2 version string.",
    )
    .optflag(
        VERBOSE_SHORT,
        VERBOSE,
    ...
    .optopt(
        NAME_SHORT,
        NAME,
        "Device name. Defaults to spotify_iv2.",
        "NAME",
    )
    .optopt(
        BITRATE_SHORT,
    ...
    let stripped_env_key = |k: &str| {
        k.trim_start_matches("spotify_iv2_")
            .replace('_', "-")
            .to_lowercase()
    };

    let env_vars: Vec<_> = env::vars_os().filter_map(|(k, v)| match k.into_string() {
        Ok(key) if key.starts_with("spotify_iv2_") => {
    ...
    if !env_vars.is_empty() {
        ...
        for (k, v) in &env_vars {
            if matches!(k.as_str(), "spotify_iv2_PASSWORD" | "spotify_iv2_USERNAME") {
                trace!("\t\t{k}=\"XXXXXXXX\"");
            } else if v.is_empty() {
                trace!("\t\t{k}=");
            } else {
    ...
    let connect_config = {
        ...
        #[cfg(feature = "pulseaudio-backend")]
        {
            ...
            if env::var("PULSE_PROP_application.process.binary").is_err() {
                env::set_var("PULSE_PROP_application.process.binary", "spotify_iv2");
            }
...
async fn main() {
    ...
    if setup.enable_discovery {
        ...
        match spotify_iv2::discovery::Discovery::builder(device_id, client_id)
